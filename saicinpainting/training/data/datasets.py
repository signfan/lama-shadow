...
from saicinpainting.training.data.shadow_augment import darken
...
class InpaintingAugmentedShadowTrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        dst = np.clip(img*255, 0, 255).astype('uint8')
        dst = np.transpose(dst, (1, 2 ,0))
        dst = cv2.cvtColor(dst, cv2.COLOR_RGB2BGR)
        cv2.imwrite(f'/home/shk/Work/gen_shadow/{os.path.basename(tpath)}', dst)
